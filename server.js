...
// Advanced Social Media Scraping with Post Extraction
async function scrapeSocialMediaProfile(url, platform) {
    ...
    try {
        ...
        // Enable request interception to handle dynamic content
        ...
        const profileData = await page.evaluate((platform) => {
            ...
            const platformSelectors = {
                tiktok: {
                    ...
                    postSelector: '.tiktok-1s4oj7j-DivVideoFeedV2',
                    postImage: 'img.tiktok-1k7z6p-ImgPoster',
                    postLikes: '[data-e2e="video-player-like"] strong',
                    postComments: '[data-e2e="video-player-comment"] strong'
                },
                instagram: {
                    ...
                    postImage: 'img[alt*="media by"]',
                    postLikes: '._aada span',
                    postComments: '._aada + span'
                },
                twitter: {
                    profilePic: 'img[alt="Profile picture"]',
                    ...
                    postLikes: '[data-testid="like"]',
                    postComments: '[data-testid="reply"]'
                },
                facebook: {
                    ...
                    postSelector: 'div.x1yztbdb',
                    postImage: 'img.x1n2onr6',
                    postLikes: 'span.x1e558r4',
                    postComments: 'span.x1e558r4 + span'
                },
                youtube: {
                    profilePic: 'img#img',
                    ...
                    postSelector: 'ytd-grid-video-renderer',
                    postImage: 'img#img',
                    postLikes: '#vote-count-middle',
                    postComments: '#comment-count'
                },
                linkedin: {
                    ...
                    postSelector: '.share-box-feed-entry',
                    postImage: 'img.feed-shared-image',
                ...
                github: {
                    ...
                    postSelector: '.js-repos-container .repo',
                    postImage: '',
                    postLikes: '.stargazers',
                    postComments: '.issues'
                },
                reddit: {
                    ...
                    posts: '.Post',
                    postSelector: '.Post',
                    postImage: 'img._3rJ6s',
                    postLikes: '.score.unvoted',
                    postComments: '.numComments'
                }
            };

            const selectors = platformSelectors[platform] || {};
            const posts = Array.from(document.querySelectorAll(selectors.postSelector)).slice(0, 10).map(post => {
                const postEl = post;
                return {
                    content: getText(`${selectors.postSelector} p, ${selectors.postSelector} span`) || 'N/A',
                    image: getImage(selectors.postImage) || null,
                    likes: getText(selectors.postLikes) || '0',
                    comments: getText(selectors.postComments) || '0',
                    timestamp: postEl.querySelector('time')?.getAttribute('datetime') || new Date().toISOString()
                };
            });
...
async function performComprehensiveSearch(username) {
    const results = {
        timestamp: new Date().toソーシャルメディア
        username,
        social_media: {},
        general_search: [],
        email_search: [],
            ...
            section.forEach(item => {
                ...
                if (item.posts) {
                    item.posts.forEach(post => {
                        if (post.content) allText += ` ${post.content}`;
                    });
                }
            });
...
app.get('/health', (req, res) => {
    res.json({ 
        ...
        features: [
            ...
            'Phone Number Search',
            'Post Content & Engagement Analysis'
        ],
